...
if platform.system() == "Windows":
    try:
        ...
        print("⚠️ Windows-specific modules not available")
...
# Speech queue (if audio available)
...
def speak(text):
    """Queue text for speech synthesis or print to console."""
    print(f"[Agent]: {text}")
    if audio_initialized:
...
def get_detailed_screen_context(title, process_name, platform, cmdline=None):
    ...
    if 'python' in process_lower:
        if 'agent_w' in cmdline_str:
            detailed_info += f"The user is running their voice assistant application in Ubuntu Linux environment.\n"
            detailed_info += f"Command: {cmdline_str}\n"
            detailed_info += f"I can help with: voice assistant features, AI integrations, Python development, debugging their current application, Ubuntu Linux tasks.\n"
        else:
            detailed_info += f"The user is running a Python application in Linux Terminal.\n"
            detailed_info += f"Command: {cmdline_str}\n"
    ...
    # Default case
        ...
        detailed_info += f"The user is working with {process_name} in {platform} environment.\n"
...
def listen_and_respond_audio(recognizer):
...
def listen_and_respond_text():
    """Listen for text input and respond."""
    global total_tokens
    try:
        user_input = input("\n💬 Type your message (or 'stop' to exit): ").strip()
        
        if not user_input:
            return True
            
        input_tokens = count_tokens(user_input)
        print(f"💭 You typed: {user_input} (🔢 {input_tokens} tokens)")
...
class ConsoleAssistant:
    def __init__(self):
        ...
        if audio_initialized:
            print("   • 🎤 Voice input and speech output enabled")
            self.recognizer = sr.Recognizer()
            self.use_audio = True
        else:
            print("   • ⌨️ Text input mode (audio not available)")
        ...
        if self.use_audio:
            threading.Thread(target=self.listen_loop_audio, daemon=True).start()
        else:
            threading.Thread(target=self.listen_loop_text, daemon=True).start()
        ...
        try:
            ...
            if self.use_audio:
                ...
                contextual_greeting = f"Hello! I can see you're working with {initial_context['process']} in a Linux environment. I understand your current context and I'm ready to provide specific help. Since audio isn't available, I'll work with text input instead."
            
            speak(contextual_greeting)
            
        except Exception as e:
            print(f"⚠️ Initial context error: {e}")
            speak("Hello! I'm your enhanced screen-aware assistant. I can analyze what you're currently doing and provide contextual help.")
    ...
    def update_window_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"🪟 Active: '{info['title']}'")
                print(f"🔧 Process: {info['process']} ({info.get('platform', 'Unknown')})")
                print(f"📋 Context: {info['context']}")
                
                time.sleep(15)  # Longer interval
                
            except Exception as e:
                print(f"⚠️ Window monitoring error: {e}")
                time.sleep(15)

    def listen_loop_audio(self):
        """Continuously listen for audio input."""
        while self.running:
            if not listen_and_respond_audio(self.recognizer):
                self.running = False
                break
            time.sleep(0.1)

    def listen_loop_text(self):
        """Continuously listen for text input."""
        while self.running:
            if not listen_and_respond_text():
                self.running = False
                break
            time.sleep(0.1)
    ...
    def run(self):
        try:
            if self.use_audio:
                ...
                print("\n⌨️ Enhanced Text Assistant Ready!")
                print("💬 I can now understand what you're working on and provide contextual help.")
                print("📝 Just type your questions - I'll respond based on your current activity!")
